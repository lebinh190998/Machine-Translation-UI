...
from demo.ctpn.text_detect import text_detect
from demo.lib.fast_rcnn.config import cfg_from_file
...
def charRec(img, text_recs, adjust=False):
   ...
   """
    ...
    for index, rec in enumerate(text_recs):
        ...
        if partImg.shape[0] < 1 or partImg.shape[1] < 1 or partImg.shape[0] > partImg.shape[1]:  # filter abnormal pictures
            continue
        ...
        if len(text) > 0:
            results[index] = [rec]
            results[index].append(text)  # recognized text
...
def model(img, adjust=False):
    """
    @img: image
    @adjust: whether to adjust the text recognition result
    """
